...
export function CanvasNodeHoverToolbar({
    ...
    const canRetry = node.metadata?.status === "error" && node.type !== CanvasNodeType.Thinking;
                ...
                <div className="h-[56vh] min-h-[360px] text-sm">
                    {view === "info" ? (
                        <div className="thin-scrollbar h-full space-y-3 overflow-auto pr-1">
                            ...
                            <InfoRow label="类型" value={node.type === CanvasNodeType.Text ? "文本" : node.type === CanvasNodeType.Image ? "图片" : node.type === CanvasNodeType.Video ? "视频" : node.type === CanvasNodeType.Audio ? "音频" : node.type === CanvasNodeType.Group ? "组" : node.type === CanvasNodeType.Thinking ? "AI 思维" : "生成配置"} />
