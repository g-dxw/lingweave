...
type CanvasNodeData = {
  id: string;
  type: "image" | "text" | "config" | "thinking" | "video" | "audio" | "group";
  title: string;
...
- `id`：节点 ID。
- `type`：节点类型，当前有图片、文本、生成配置、AI 思维、视频、音频和组七类。
- `title`：节点标题。
...
type CanvasNodeMetadata = {
  ...
  thinkingMode?: "diverge" | "sequence" | "outline";
...
- 音频节点：`content` 是当前可播放的音频 URL；`storageKey` 指向本地音频 Blob；`durationMs/bytes/mimeType` 保存媒体信息。
- 文本节点：`content` 保存文本内容；`fontSize` 保存字体大小；`prompt/status/errorDetails` 保存生成状态。
- 生成配置节点：`composerContent/generationMode/model/size/count` 保存提示词组装和生成配置；`generationMode` 可选择文本、图片、视频或音频；上游输入通过 `connections` 计算。
- AI 思维节点：`thinkingMode/thinkingCount` 保存结构和数量；生成的文本节点通过 `thinkingRootId/thinkingRunId/thinkingIndex` 记录所属思维节点、运行批次和顺序。
- 组节点：自身不保存媒体内容；组内普通节点通过 `groupId` 记录归属。
...
AI 思维节点的发散和总分结构由思维节点连接所有文本子节点；序列结构由思维节点连接第一个文本节点，后续文本节点依次链式连接。连线既表达画布关系，也决定生成配置和 AI 思维节点读取哪些上游素材。
